...
let viewValue: 'temperature' | 'humidity';
...
let mapRange = (numToMap:number, in_min: number, in_max: number, out_min: number, out_max: number) => (numToMap - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
...
function setGradient(type: 'temperature' | 'humidity') {
    let gradientString = '';
    if(type === 'temperature') {
